...
int main() {
    ...
    ndspWaveBuf sPsst={0}, sAttack={0}, sCaught={0}, sDoor={0}, sLockedDoor={0}, sDupeAttack={0};
    ndspWaveBuf sRushScream={0}, sEyesAppear={0}, sEyesGarble={0}, sEyesAttack={0}, sEyesHit={0};
    ndspWaveBuf sSeekRise={0}, sSeekChase={0}, sSeekEscaped={0}, sDeath={0}, sElevatorJam={0}, sElevatorJamEnd={0};
    ndspWaveBuf sCoinsCollect={0}, sDarkRoomEnter={0}, sDrawerClose={0}, sDrawerOpen={0};
    ...
    while (aptMainLoop()) {
        ...
        if (gameState == 1) {
            ...
            if (inElevator && !elevatorDoorsOpen) { 
                ...
                if (audio_ok && sElevatorJam.data_vaddr) {
                    if (elevatorTimer < 794 && sElevatorJam.status == NDSP_WBUF_DONE && !elevatorJamFinished) {
                        timeToOpen = true;
                    }
            ...
            if (playerCurrentRoom >= 0 && playerCurrentRoom < TOTAL_ROOMS) {
                ...
                for (int s = 0; s < 3; s++) {
                    if (stepAnim(rooms[playerCurrentRoom].drawerOpen[s], rooms[playerCurrentRoom].animMain[s])) {
                        animActive = true; 
                    }
                }
            ...
            if (totalFrames % 5 == 0) {
                ...
                if (isDead) { 
                    ...
                    if (nD >= 0 && nD < TOTAL_ROOMS) { 
                        if (isGlitch && tDR == nD) { 
                            float dX[3], dZ[3];
                            rotateVertexRelative(-5.0f, 0.0f, rooms[tDR].centerX, rooms[tDR].centerZ, rooms[tDR].orientation, dX[0], dZ[0]); 
                            rotateVertexRelative(0.0f, -5.0f, rooms[tDR].centerX, rooms[tDR].centerZ, rooms[tDR].orientation, dX[1], dZ[1]); 
                            rotateVertexRelative(5.0f, 0.0f, rooms[tDR].centerX, rooms[tDR].centerZ, rooms[tDR].orientation, dX[2], dZ[2]);  
                            
                            int lookedAt = -1;
                            for(int d = 0; d < 3; d++) {
                                if (fabsf(camX - dX[d]) < 3.0f && fabsf(camZ - dZ[d]) < 3.0f) { 
                                    lookedAt = d; 
                                    break; 
                                }
                            }
                    ...
                    printf("       --- CONTROLS ---       \x1b[K\n");
                    printf(" [A] Interact  [B] Crouch    \x1b[K\n");
                    printf(" [X] Hide(Cab/Bed) [CPAD] Move \x1b[K\n");
                    printf(" [TOUCH/L/R/CSTICK] Look     \x1b[K\n");
                    
                    if (texErrorMessage[0] != '\0') {
                        printf("\n \x1b[31m[TEX ERROR] %s\x1b[0m \x1b[K\n", texErrorMessage);
                    }
                    
                    if (messageTimer > 0) {
                        printf("\n ** %s ** \x1b[K\n", uiMessage);
                    } else if (rushActive && rushState == 1) {
                        printf("\n ** The lights are flickering... ** \x1b[K\n");
                    } else if (hideState == BEHIND_DOOR) {
            ...
            if (!isDead) {
                ...
                if (seekState == 1) { 
                    ...
                    if (sqrtf(pdx * pdx + pdz * pdz) < 1.2f) {
                        playerHealth = 0; 
                        isDead = true; 
                        sprintf(uiMessage, "Seek caught you..."); 
                        messageTimer = 60;
                        ...
                        for(int h = 0; h < 6; h++) {
                            if (fabsf(camX - rooms[playerCurrentRoom].pW[h]) < 0.6f && fabsf(camZ - rooms[playerCurrentRoom].pZ[h]) < 0.6f) {
                                if (rooms[playerCurrentRoom].pSide[h] == 0 && !isDead && messageTimer <= 0) {
                                    playerHealth -= 40; 
                                    flashRedFrames = 15; 
                                    sprintf(uiMessage, "Burned! (-40 HP)"); 
                                    messageTimer = 30;
                                    if (playerHealth <= 0) { 
                                        isDead = true; 
                                        if (audio_ok) ndspChnWaveBufClear(7); 
                                    ...
                                    playerHealth = 0; 
                                    isDead = true; 
                                    sprintf(uiMessage, "The hands grabbed you!"); 
                                    messageTimer = 60;
                                    if (audio_ok) ndspChnWaveBufClear(7);
                                }
                            }
                    ...
                    if (seekActive) { 
                        ...
                        if (playerSafe || seekZ < fLZ + 3.0f) { 
                            if (!rooms[sR].isJammed) {
                                doorOpen[sR] = false; 
                                rooms[sR].isLocked = false; 
                                rooms[sR].isJammed = true; 
                                needsVBOUpdate = true;
                                ...
                                if (audio_ok) {
                                    ...
                                    if (sSeekEscaped.data_vaddr) {
                                        sSeekEscaped.status = NDSP_WBUF_FREE; 
                                        ndspChnWaveBufAdd(7, &sSeekEscaped);
                                    }
                                }
                            }
                        } 
                        if (playerSafe) {
                            seekActive = false; 
                            seekState = 0; 
                            sprintf(uiMessage, "You escaped!"); 
                            messageTimer = 75; 
                            needsVBOUpdate = true;
                        } else if (seekZ < fLZ + 3.0f) {
                            playerHealth = 0; 
                            isDead = true; 
                            sprintf(uiMessage, "The door slammed shut!"); 
                            messageTimer = 60; 
                            seekActive = false; 
                            seekState = 0; 
                            needsVBOUpdate = true;
                ...
                if (inE && !inEyesRoom) { 
                    ...
                    if (audio_ok) {
                        ...
                        if (sEyesAppear.data_vaddr) { 
                            sEyesAppear.status = NDSP_WBUF_FREE; 
                            ndspChnWaveBufAdd(4, &sEyesAppear); 
                        }
                        ndspChnWaveBufClear(5);
                        if (sEyesGarble.data_vaddr) { 
                            ...
                            m[1] = 1.8f; 
                            ndspChnSetMix(5, m); 
                            sEyesGarble.status = NDSP_WBUF_FREE; 
                            ndspChnWaveBufAdd(5, &sEyesGarble); 
                        }
                    } 
                            ...
                            if (eyesGraceTimer <= 0) { 
                                if (!isLookingAtEyes) {
                                    ...
                                    if (audio_ok && sEyesAttack.data_vaddr && eyesSoundCooldown <= 0) {
                                        ndspChnWaveBufClear(4); 
                                        sEyesAttack.status = NDSP_WBUF_FREE; 
                                        ndspChnWaveBufAdd(4, &sEyesAttack);
                                ...
                                if (eyesDamageTimer >= 3) {
                                    ...
                                    if (eyesDamageAccumulator >= 5) {
                                        ...
                                        if (audio_ok && sEyesHit.data_vaddr) {
                                            ...
                                            sEyesHit.status = NDSP_WBUF_FREE; 
                                            ndspChnWaveBufAdd(6, &sEyesHit);
                                        }
                                    }
                                } 
                                if (playerHealth <= 0) {
                ...
                if (!screechActive && screechCooldown <= 0 && hideState == NOT_HIDING && playerCurrentRoom > 0 && !iSE && !inLibrary && (rand() % sC == 0)) { 
                    ...
                    if (audio_ok) {
                        ...
                        if (sPsst.data_vaddr) { 
                            sPsst.status = NDSP_WBUF_FREE; 
                            ndspChnWaveBufAdd(0, &sPsst); 
                        }
                    } 
                }
                ...
                if (screechActive) { 
                    if (screechState == 1) {
                        screechTimer--; 
                        float cOX = screechOffsetX;
                        float cOZ = screechOffsetZ; 
                        float pY = isCrouching ? 0.4f : 0.9f; 
                        float nSX = camX + cOX;
                        float nSZ = camZ + cOZ;
                        float nSY = 0.8f + screechOffsetY;
                        
                        int att = 0; 
                        float currentDist = 1.2f; 
                        ...
                        if (dP > 0.85f) { 
                            screechState = 2; 
                            screechTimer = 15; 
                            sprintf(uiMessage, "Dodged Screech!"); 
                            messageTimer = 45; 
                            if (audio_ok) { 
                                ndspChnWaveBufClear(0); 
                                if (sCaught.data_vaddr) { 
                                    sCaught.status = NDSP_WBUF_FREE; 
                                    ndspChnWaveBufAdd(0, &sCaught); 
                                } 
                            } 
                            ...
                            screechTimer = 24; 
                            playerHealth -= 20; 
                            sprintf(uiMessage, "Screech bit you! (-20 HP)"); 
                            messageTimer = 45; 
                            if (playerHealth <= 0) isDead = true; 
                            
                            if (audio_ok) { 
                                ndspChnWaveBufClear(0); 
                                if (sAttack.data_vaddr) { 
                                    sAttack.status = NDSP_WBUF_FREE; 
                                    ndspChnWaveBufAdd(0, &sAttack); 
                                } 
                            } 
                        } 
                        ...
                        screechTimer--; 
                        screechOffsetX = -sinf(camYaw) * 0.4f; 
                        screechOffsetZ = -cosf(camYaw) * 0.4f; 
                        screechX = camX + screechOffsetX; 
                        screechZ = camZ + screechOffsetZ; 
                        ...
                        if (screechTimer > 18) {
                            flashRedFrames = 0; 
                        } else if (screechTimer > 12) {
                            flashRedFrames = 2; 
                        } else if (screechTimer > 6) {
                            flashRedFrames = 0; 
                        } else {
                            flashRedFrames = 2;
                        }
                ...
                if ((kDown & KEY_B) && hideState == NOT_HIDING) { 
                    if (isCrouching) { 
                        if (!checkCollision(camX, 0, camZ, 1.1f)) {
                            isCrouching = false; 
                        }
                    } else { 
                        isCrouching = true; 
                ...
                if (playerCurrentRoom > 1 && !rushActive && rushCooldown <= 0 && !iSE && !inLibrary && rand() % 10000 < 25) {
                    rushActive = true; 
                    rushState = 1; 
                    rushTimer = 150 + (rand() % 60); 
                    rushStartTimer = (float)rushTimer;
                    
                    if (audio_ok) {
                        if (sLightsFlicker.data_vaddr) { 
                            float m[12] = {0}; 
                            m[0] = 2.5f; 
                            m[1] = 2.5f; 
                            ndspChnSetMix(12, m); 
                            ndspChnWaveBufClear(12); 
                            sLightsFlicker.status = NDSP_WBUF_FREE; 
                            ndspChnWaveBufAdd(12, &sLightsFlicker); 
                        }
                        if (sRushScream.data_vaddr) { 
                            float m[12] = {0}; 
                            ndspChnSetMix(3, m); 
                            ndspChnWaveBufClear(3); 
                            sRushScream.status = NDSP_WBUF_FREE; 
                            ndspChnWaveBufAdd(3, &sRushScream); 
                        }
                    }
                }
                ...
                if (hideState == NOT_HIDING || hideState == BEHIND_DOOR) { 
                    ...
                    if (iDZ && hideState == NOT_HIDING) {
                        hideState = BEHIND_DOOR; 
                    } else if (!iDZ && hideState == BEHIND_DOOR) {
                ...
                if ((hideState == NOT_HIDING || hideState == BEHIND_DOOR) && seekState != 1) {
                    ...
                    if (kHeld & KEY_TOUCH) { 
                        if (!wasTouching) { 
                            ...
                            float dx = (float)t.px - startTouchX;
                            float dy = (float)t.py - startTouchY;
                            
                            if (fabsf(dx) < 10.0f) dx = 0; 
                            if (fabsf(dy) < 10.0f) dy = 0;
                ...
                if (rushActive) { 
                    if (rushState == 1) {
                        ...
                        rushZ -= 1.6f;
                        if (playerCurrentRoom >= 0 && playerCurrentRoom < TOTAL_ROOMS) {
                            if (rooms[playerCurrentRoom].lightLevel != 0.3f) { 
                ...
                if (kDown & KEY_X && hideState == NOT_HIDING) { 
                    ...
                    for (auto& b : collisions) { 
                        if ((b.type == 1 || b.type == 2) && camX + r > b.minX && camX - r < b.maxX && camZ + r > b.minZ && camZ - r < b.maxZ) { 
                            float rCZ = (b.minZ + b.maxZ) / 2.0f;
                            float cCX = ((b.minX + b.maxX) / 2.0f);
                            ...
                            if (audio_ok && sWardrobeEnter.data_vaddr) { 
                                ndspChnWaveBufClear(10); 
                                sWardrobeEnter.status = NDSP_WBUF_FREE; 
                                ndspChnWaveBufAdd(10, &sWardrobeEnter); 
                            } 
                            break; 
                        } 
                    } 
                    ...
                    if (hideState == IN_CABINET || hideState == UNDER_BED) {
                        ...
                        if (audio_ok && sWardrobeExit.data_vaddr) { 
                            ndspChnWaveBufClear(10); 
                            sWardrobeExit.status = NDSP_WBUF_FREE; 
                            ndspChnWaveBufAdd(10, &sWardrobeExit); 
                        }
                    } 
                }
                    ...
                    if (nI >= 0 && nI < TOTAL_ROOMS && rooms[nI].isDupeRoom && nI != LIBRARY_ROOM && nI != LIBRARY_ROOM + 1) { 
                        ...
                        if (tD != -1) { 
                            if (tD == rooms[nI].correctDupePos) { 
                                ...
                                if (audio_ok && sDupeAttack.data_vaddr) { 
                                    ndspChnWaveBufClear(2); 
                                    sDupeAttack.status = NDSP_WBUF_FREE; 
                                    ndspChnWaveBufAdd(2, &sDupeAttack); 
                                }
                                playerHealth -= 34; 
                                flashRedFrames = 8; 
                ...
                if ((kDown & KEY_A) || (kDown & KEY_X && hideState == NOT_HIDING)) {
                    ...
                    auto cI = [&](int ty, float zC, float sX, bool& iO, int& it) { 
                        if (ty != 0) {
                            ...
                            if (distSq < 6.25f) {
                                ...
                                if (dot > 0.7f) {
                                    float aX = sX - (dx / dist) * 0.5f;
                                    float aZ = zC - (dz / dist) * 0.5f; 
                                    ...
                                    if (checkLineOfSight(camX, (isCrouching ? 0.4f : 0.9f), camZ, aX, targetY, aZ)) {
                                        if (kDown & KEY_X && (ty == 5 || ty == 6)) {
                                            ...
                                            if (audio_ok) {
                                                ndspChnWaveBufClear(10);
                                                if (iO && sDrawerOpen.data_vaddr) { 
                                                    sDrawerOpen.status = NDSP_WBUF_FREE; 
                                                    ndspChnWaveBufAdd(10, &sDrawerOpen); 
                                                }
                                                else if (!iO && sDrawerClose.data_vaddr) { 
                                                    sDrawerClose.status = NDSP_WBUF_FREE; 
                                                    ndspChnWaveBufAdd(10, &sDrawerClose); 
                                                }
                                            }
                                            return true;
                                        } 
                                        if (kDown & KEY_A) { 
                                            if ((ty == 5 || ty == 6) && iO) {
                                                if (it == 1) { 
                                                    ...
                                                    it = 0; 
                                                    needsVBOUpdate = true; 
                                                    sprintf(uiMessage, "Grabbed the Golden Key!"); 
                                                    messageTimer = 45; 
                                                    return true; 
                                                }
                                                else if (it == 2) { 
                                                    ...
                                                    it = 0; 
                                                    needsVBOUpdate = true; 
                                                    sprintf(uiMessage, "Used a Bandaid! (+10 HP)"); 
                                                    messageTimer = 45; 
                                                    return true; 
                                                }
                                                else if (it == 3) { 
                                                    playerCoins += (rand() % 15) + 5; 
                                                    it = 0; 
                                                    needsVBOUpdate = true; 
                                                    sprintf(uiMessage, "Looted some Coins!"); 
                                                    messageTimer = 45; 
                                                    ...
                                                    sprintf(uiMessage, "Drawer is empty..."); 
                                                    messageTimer = 30; 
                                                    return true; 
                                                }
                                            } else if ((ty == 3 || ty == 4)) {
                                                if (it == 1) { 
                                                    ...
                                                    it = 0; 
                                                    needsVBOUpdate = true; 
                                                    sprintf(uiMessage, "Grabbed Key off the bed!"); 
                                                    messageTimer = 45; 
                                                    return true; 
                                                }
                                                else if (it == 3) { 
                                                    ...
                                                    it = 0; 
                                                    needsVBOUpdate = true; 
                                                    sprintf(uiMessage, "Looted Coins off the bed!"); 
                                                    messageTimer = 45; 
                                                    if (audio_ok && sCoinsCollect.data_vaddr) { 
                                                        ndspChnWaveBufClear(10); 
                                                        sCoinsCollect.status = NDSP_WBUF_FREE; 
                    ...
                    auto cC = [&](float cX, float cZ, bool& iO) { 
                        if ((kDown & KEY_X) && !iO) {
                            ...
                            if (distSq < 6.25f) {
                                ...
                                if (dot > 0.7f) {
                                    float aX = cX - (dx / dist) * 0.5f;
                                    float aZ = cZ - (dz / dist) * 0.5f;
                                    
                                    if (checkLineOfSight(camX, (isCrouching ? 0.4f : 0.9f), camZ, aX, 0.3f, aZ)) {
                                        iO = true; 
                                        needsVBOUpdate = true; 
                                        playerCoins += 20; 
                                        sprintf(uiMessage, "Opened Chest!"); 
                                        messageTimer = 45;
                                        if (audio_ok && sDrawerOpen.data_vaddr) { 
                                            ndspChnWaveBufClear(10); 
                                            sDrawerOpen.status = NDSP_WBUF_FREE; 
                                            ndspChnWaveBufAdd(10, &sDrawerOpen); 
                                        }
                    ...
                    if (!iA && playerCurrentRoom >= 0 && playerCurrentRoom < TOTAL_ROOMS) { 
                        for (int s = 0; s < 3; s++) {
                            ...
                            if (cI(rooms[playerCurrentRoom].slotType[s], gZ, gX, rooms[playerCurrentRoom].drawerOpen[s], rooms[playerCurrentRoom].slotItem[s])) { 
                                iA = true; 
                                break; 
                            }
                        }
                        ...
                        auto checkSideRoomInteracts = [&](bool hasRoom, Direction wallDir, float doorOffset, int* typeL, int* itemL, bool* openL, float* animL, int* typeR, int* itemR, bool* openR, float* animR) {
                            ...
                            for (int s = 0; s < 3; s++) {
                                ...
                                if (typeL[s] > 0 && typeL[s] != 99) {
                                    float gX, gZ; 
                                    rotateVertexRelative(doorOffset - 2.4f, lz, rooms[playerCurrentRoom].centerX, rooms[playerCurrentRoom].centerZ, wallDir, gX, gZ);
                                    
                                    if (typeL[s] == 7 || typeL[s] == 8) { 
                                        if (cC(gX, gZ, openL[s])) return true; 
                                    } else if (cI(typeL[s], gZ, gX, openL[s], itemL[s])) {
                                        return true; 
                                    }
                                }
                                ...
                                if (typeR[s] > 0 && typeR[s] != 99) {
                                    ...
                                    if (typeR[s] == 7 || typeR[s] == 8) { 
                                        if (cC(gX, gZ, openR[s])) return true; 
                                    } else if (cI(typeR[s], gZ, gX, openR[s], itemR[s])) {
                                        return true; 
                                    }
                                }
                            }
                        ...
                        for (int i = 0; i < TOTAL_ROOMS; i++) { 
                            ...
                            if (rooms[i].isLocked || rooms[i].isJammed) { 
                                ...
                                if (fabsf(camZ - eZ) < 2.5f && fabsf(camX - eX) < 2.0f) { 
                                    if (rooms[i].isJammed) {
                                        if (audio_ok && sLockedDoor.data_vaddr) { 
                                            ndspChnWaveBufClear(1); 
                                            sLockedDoor.status = NDSP_WBUF_FREE; 
                                            ndspChnWaveBufAdd(1, &sLockedDoor); 
                                        }
                                        sprintf(uiMessage, "The door is jammed shut!"); 
                                        messageTimer = 30;
                                    } else if (hasKey) {
                                        rooms[i].isLocked = false; 
                                        hasKey = false; 
                                        needsVBOUpdate = true; 
                                        sprintf(uiMessage, "Door Unlocked!"); 
                                        messageTimer = 30;
                                    } else {
                                        if (audio_ok && sLockedDoor.data_vaddr) { 
                                            ndspChnWaveBufClear(1); 
            ...
            if (playerCurrentRoom >= 0 && playerCurrentRoom < TOTAL_ROOMS) {
                auto checkSideDoor = [&](bool hasRoom, Direction wallDir, float doorOffset, bool& doorOpenState) {
                    if (hasRoom && !doorOpenState) {
                        float gX, gZ; 
                        rotateVertexRelative(doorOffset, -4.4f, rooms[playerCurrentRoom].centerX, rooms[playerCurrentRoom].centerZ, wallDir, gX, gZ);
                        
                        if (fabsf(camZ - gZ) < 2.0f && fabsf(camX - gX) < 2.0f) { 
                            doorOpenState = true; 
                            needsVBOUpdate = true; 
                            if (audio_ok && sDoor.data_vaddr) { 
                                ndspChnWaveBufClear(1); 
                ...
                if (playerCurrentRoom == -1) { 
                    if (hasSeekRunAnim) {
                        ...
                        if (test < 50) {
                            ...
                            sprintf(uiMessage, current_frame_text);
                            messageTimer = 10;
                            seekModel.draw(seekModel, 0, 0.0f, 0.0f + seekHeightAdjust, 2.0f, seekScale, 1.0f, 3.14159f);
                            test += 1;
                        }
        ...
        if (gameState == 2) {
            ...
            if (seekState == 1) { 
                ...
                if (seekTimer <= 65) {
                    float t = seekTimer / 65.0f; 
                    t = t * t * (3.0f - 2.0f * t); 
                    dCZ = camZ + (lCCZ - camZ) * t; 
                    dCY = camYaw + (3.14159f - camYaw) * t;
                } else {
                    dCZ = lCCZ; 
                    dCY = 3.14159f; 
