...
int main() {
    ...
    while (aptMainLoop()) {
        ...
        if(!isDead){
            ...
            if(figureActive) { float distToPlayer = sqrt(pow(camX - figureX, 2) + pow(camZ - figureZ, 2)); if(figureState == 0) { if(!isCrouching && distToPlayer < 4.0f) figureState = 2; } else if(figureState == 2) { if(camX > figureX) figureX += figureSpeed; else figureX -= figureSpeed; if(camZ > figureZ) figureZ += figureSpeed; else figureZ -= figureSpeed; if(distToPlayer < 0.8f) { playerHealth = 0; isDead = true; flashRedFrames = 50; sprintf(uiMessage, "The Figure found you..."); messageTimer = 120; } needsVBOUpdate = true; } }
            ...
            if(screechActive){ 
                ...
                float vx=screechX-camX, vy=screechY-(isCrouching?0.4f:0.9f), vz=screechZ-camZ, dist=sqrt(vx*vx+vy*vy+vz*vz); if(dist>0.0f){vx/=dist;vy/=dist;vz/=dist;} float fx=-sinf(camYaw)*cosf(camPitch), fy=sinf(camPitch), fz=-cosf(camYaw)*cosf(camPitch), dP=(fx*vx)+(fy*vy)+(fz*vz); 
