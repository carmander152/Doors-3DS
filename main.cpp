...
static const vertex level_mesh[] = {
	// --- 1. HALLWAY (Floor, Walls, Roof) [Index 0-35] ---
	...
	// --- 2. ROOM (Floor, Walls, Roof) [Index 36-71] ---
	...
	// --- 3. ELEVATOR Interior [Index 72-107] ---
	{{ -1.0f, 0.0f, 3.0f, 1.0f }, { 0.2f, 0.2f, 0.2f, 1.0f }}, {{  1.0f, 0.0f, 3.0f, 1.0f }, { 0.2f, 0.2f, 0.2f, 1.0f }}, {{ -1.0f, 0.0f, 5.0f, 1.0f }, { 0.2f, 0.2f, 0.2f, 1.0f }},
	{{ -1.0f, 1.5f, 5.0f, 1.0f }, { 0.3f, 0.3f, 0.3f, 1.0f }}, {{  1.0f, 1.5f, 5.0f, 1.0f }, { 0.3f, 0.3f, 0.3f, 1.0f }}, {{ -1.0f, 0.0f, 5.0f, 1.0f }, { 0.3f, 0.3f, 0.3f, 1.0f }},
	{{ -1.0f, 0.0f, 3.0f, 1.0f }, { 0.3f, 0.3f, 0.3f, 1.0f }}, {{ -1.0f, 1.5f, 3.0f, 1.0f }, { 0.3f, 0.3f, 0.3f, 1.0f }}, {{ -1.0f, 0.0f, 5.0f, 1.0f }, { 0.3f, 0.3f, 0.3f, 1.0f }},
	{{  1.0f, 0.0f, 3.0f, 1.0f }, { 0.3f, 0.3f, 0.3f, 1.0f }}, {{  1.0f, 1.5f, 3.0f, 1.0f }, { 0.3f, 0.3f, 0.3f, 1.0f }}, {{  1.0f, 0.0f, 5.0f, 1.0f }, { 0.3f, 0.3f, 0.3f, 1.0f }},

	// --- 4. ELEVATOR DOORS [Index 108-119] ---
	{{ -1.0f, 0.0f, 3.01f, 1.0f }, { 0.4f, 0.4f, 0.45f, 1.0f }}, {{  0.0f, 0.0f, 3.01f, 1.0f }, { 0.4f, 0.4f, 0.45f, 1.0f }}, {{ -1.0f, 1.5f, 3.01f, 1.0f }, { 0.4f, 0.4f, 0.45f, 1.0f }},
	{{  0.0f, 0.0f, 3.01f, 1.0f }, { 0.0f, 1.5f, 3.01f, 1.0f }, { 0.4f, 0.4f, 0.45f, 1.0f }}, {{ -1.0f, 1.5f, 3.01f, 1.0f }, { 0.4f, 0.4f, 0.45f, 1.0f }},
	{{  0.0f, 0.0f, 3.01f, 1.0f }, {  1.0f, 0.0f, 3.01f, 1.0f }, { 0.4f, 0.4f, 0.45f, 1.0f }}, {{  0.0f, 1.5f, 3.01f, 1.0f }, { 0.4f, 0.4f, 0.45f, 1.0f }},

	// --- 5. CABINET Body [Index 120-143] ---
	...
	// --- 6. CABINET DOORS (Crack) [Index 144-155] ---
...
int main() {
	...
	int uLoc_model = shaderInstanceGetUniformLocation(program.vertexShader, "model_mtx"); // Needed for doors
	...
	while (aptMainLoop()) {
		...
		frames++;
		if (frames > 60 && doorSlide < 0.9f) doorSlide += 0.01f;
		...
		// DRAWING
		C3D_DrawArrays(GPU_TRIANGLES, 0, 36);   // Hallway
		C3D_DrawArrays(GPU_TRIANGLES, 36, 36);  // Room
		C3D_DrawArrays(GPU_TRIANGLES, 72, 36);  // Elevator Interior
		C3D_DrawArrays(GPU_TRIANGLES, 120, 24); // Cabinet Body
		C3D_DrawArrays(GPU_TRIANGLES, 144, 12); // Cabinet Doors
		C3D_DrawArrays(GPU_TRIANGLES, 156, 24); // Bed

		// DOOR ANIMATION
		Mtx_Identity(&model);
		Mtx_Translate(&model, -doorSlide, 0, 0, true);
		C3D_FVUnifMtx4x4(GPU_VERTEX_SHADER, uLoc_model, &model);
		C3D_DrawArrays(GPU_TRIANGLES, 108, 6); // Left Door
		
		Mtx_Identity(&model);
		Mtx_Translate(&model, doorSlide, 0, 0, true);
		C3D_FVUnifMtx4x4(GPU_VERTEX_SHADER, uLoc_model, &model);
		C3D_DrawArrays(GPU_TRIANGLES, 114, 6); // Right Door
