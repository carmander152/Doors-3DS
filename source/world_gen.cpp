...
void rotateVertexRelative(float localX, float localZ, float centerX, float centerZ, Direction dir, float& outX, float& outZ) {
    if (dir == NORTH) { outX = centerX + localX; outZ = centerZ + localZ; }
    else if (dir == EAST) { outX = centerX - localZ; outZ = centerZ + localX; }
    else if (dir == SOUTH) { outX = centerX - localX; outZ = centerZ - localZ; }
    else if (dir == WEST) { outX = centerX + localZ; outZ = centerZ - localX; }
}
...
void buildCabinet(float zC, bool isL, float L, float offX, int item, float cx, float cz, Direction dir) {
    float bX=(isL?-4.95f:4.85f)+offX, tX=(isL?-4.95f:4.15f)+offX, fX=(isL?-4.25f:4.15f)+offX, hX=(isL?fX+0.1f:fX-0.03f); 
    
    addRBox(bX, 0, zC-0.5f, 0.1f, 1.5f, 1.0f, 0.3f, 0.18f, 0.1f, false, 0, L, cx, cz, dir); 
    addRBox(tX, 1.5f, zC-0.5f, 0.8f, 0.1f, 1.0f, 0.3f, 0.18f, 0.1f, false, 0, L, cx, cz, dir); 
    addRBox(tX, 0, zC-0.5f, 0.8f, 1.5f, 0.1f, 0.3f, 0.18f, 0.1f, false, 0, L, cx, cz, dir); 
    addRBox(tX, 0, zC+0.4f, 0.8f, 1.5f, 0.1f, 0.3f, 0.18f, 0.1f, false, 0, L, cx, cz, dir); 
    addRBox(fX, 0, zC-0.4f, 0.1f, 1.5f, 0.35f, 0.3f, 0.18f, 0.1f, false, 0, L, cx, cz, dir); 
    addRBox(fX, 0, zC+0.05f, 0.1f, 1.5f, 0.35f, 0.3f, 0.18f, 0.1f, false, 0, L, cx, cz, dir); 
    ...
    if (!isBuildingEntities) addRCol((isL?-4.9f:4.1f)+offX, 0.0f, zC-0.5f, 0.8f, 1.5f, 1.0f, 1, cx, cz, dir);
}

void buildBed(float zC, bool isL, int item, float L, float offX, float cx, float cz, Direction dir) {
    float x=(isL?-4.95f:3.55f)+offX, sX=(isL?-3.65f:3.55f)+offX, pX=(isL?-4.65f:4.15f)+offX; 
    
    addRBox(x, 0.4f, zC-1.25f, 1.4f, 0.1f, 2.5f, 0.4f, 0.1f, 0.1f, false, 0, L, cx, cz, dir); 
    addRBox(x, 0, zC+1.15f, 0.1f, 0.4f, 0.1f, 0.2f, 0.1f, 0.05f, false, 0, L, cx, cz, dir); 
    addRBox(x+1.3f, 0, zC+1.15f, 0.1f, 0.4f, 0.1f, 0.2f, 0.1f, 0.05f, false, 0, L, cx, cz, dir);
    addRBox(x, 0, zC-1.25f, 0.1f, 0.4f, 0.1f, 0.2f, 0.1f, 0.05f, false, 0, L, cx, cz, dir); 
    addRBox(x+1.3f, 0, zC-1.25f, 0.1f, 0.4f, 0.1f, 0.2f, 0.1f, 0.05f, false, 0, L, cx, cz, dir); 
    ...
    if(!isBuildingEntities) addRCol(x, 0.0f, zC-1.25f, 1.4f, 0.6f, 2.5f, 2, cx, cz, dir);
}
...
void buildDresser(float zC, bool isL, float openFactor, int item, float L, float offX, bool hasLamp, float cx, float cz, Direction dir) {
    ...
    addRBox(frX+0.05f, 0, zC-0.35f, 0.05f, 0.2f, 0.05f, 0.2f, 0.1f, 0.05f, false, 0, L, cx, cz, dir); 
    addRBox(frX+0.05f, 0, zC+0.3f, 0.05f, 0.2f, 0.05f, 0.2f, 0.1f, 0.05f, false, 0, L, cx, cz, dir);
    addRBox(frX+0.4f, 0, zC-0.35f, 0.05f, 0.2f, 0.05f, 0.2f, 0.1f, 0.05f, false, 0, L, cx, cz, dir); 
    addRBox(frX+0.4f, 0, zC+0.3f, 0.05f, 0.2f, 0.05f, 0.2f, 0.1f, 0.05f, false, 0, L, cx, cz, dir);
    ...
    if (hasLamp) buildRLamp((isL?-4.7f:4.7f)+offX, zC, L, cx, cz, dir);
    if (!isBuildingEntities) addRCol(frX, 0.0f, zC-0.4f, 0.5f, 0.5f, 0.8f, 3, cx, cz, dir);
}

void buildChest(float x, float z, float openFactor, float L, float cx, float cz, Direction dir) {
    bool isOpen = (openFactor > 0.5f);
    addRBox(x-0.4f, 0, z-0.3f, 0.8f, 0.4f, 0.6f, 0.3f, 0.15f, 0.05f, false, 0, L, cx, cz, dir); 
    addRBox(x-0.42f, 0, z-0.32f, 0.05f, 0.4f, 0.05f, 0.8f, 0.7f, 0.1f, false, 0, L, cx, cz, dir); 
    addRBox(x+0.37f, 0, z-0.32f, 0.05f, 0.4f, 0.05f, 0.8f, 0.7f, 0.1f, false, 0, L, cx, cz, dir);
...
void addRWall(bool hasDoor, bool isDoorOpen, bool isLocked, float lz, float cx, float cz, Direction dir, float L) {
    ...
    if (!hasDoor) {
        ...
        if (!isDoorOpen) { 
            ...
            if (isLocked) addRBox(0.3f, 0.6f, lz+0.05f, 0.2f, 0.2f, 0.05f, 0.8f, 0.8f, 0.8f, false, 0, L, cx, cz, dir);
        } else { 
            ...
            if (!isBuildingEntities) addRCol(-0.6f, 0.0f, lz-1.15f, 1.1f, 1.8f, 0.1f, 4, cx, cz, dir); 
        }
    }
}
...
void buildWorld(int cChunk, int pRm) {
    ...
    if (pRm == -1) { st = -1; en = doorOpen[0] ? 1 : 0; } 
    else { 
    ...
    if(st <= -1){
        ...
        if (elevatorDoorOffset < 0.05f) addBox(-0.02f, 0, 5.04f, 0.04f, 2, 0.12f, 0, 0, 0, false); 
    ...
    for(int i = st; i <= en; i++) {
        ...
        if (isInteriorVisible) {
            ...
            if (rooms[i].isDupeRoom) {
                addRWall(true, doorOpen[i] && rooms[i].correctDupePos == 0, false, -5.0f, cx, cz, (Direction)((dir + 3) % 4), wL); 
                addRWall(true, doorOpen[i] && rooms[i].correctDupePos == 1, false, -5.0f, cx, cz, dir, wL); 
                addRWall(true, doorOpen[i] && rooms[i].correctDupePos == 2, false, -5.0f, cx, cz, (Direction)((dir + 1) % 4), wL); 
            } else {
                ...
                } else addRWall(exitLeft, doorOpen[i], rooms[i].isLocked && exitLeft, -5.0f, cx, cz, (Direction)((dir + 3) % 4), wL);
                ...
                } else addRWall(exitStraight, doorOpen[i], rooms[i].isLocked && exitStraight, -5.0f, cx, cz, dir, wL);
                ...
                } else addRWall(exitRight, doorOpen[i], rooms[i].isLocked && exitRight, -5.0f, cx, cz, (Direction)((dir + 1) % 4), wL);
            }
...
void generateRooms() {
    ...
    rooms[0].centerX = 0.0f; rooms[0].centerZ = -5.0f; 
    rooms[0].orientation = NORTH; rooms[0].chosenExitSide = 1;
    rooms[0].minX = -5.0f; rooms[0].maxX = 5.0f; rooms[0].minZ = -10.0f; rooms[0].maxZ = 0.0f;
    rooms[0].isLocked = true; rooms[0].isDupeRoom = false; rooms[0].doorPos = 1;
    
    for (int i = 1; i < TOTAL_ROOMS; i++) {
        RoomSetup& prev = rooms[i - 1];
        
        rooms[i].isLocked = false; rooms[i].isJammed = false; 
        rooms[i].hasSeekEyes = false; rooms[i].isSeekHallway = false; 
        rooms[i].isSeekChase = false; rooms[i].isSeekFinale = false; 
        ...
        for (int s = 0; s < 3; s++) { 
            ...
            rooms[i].animMain[s] = 0.0f; rooms[i].slotType[s] = 0;
        }
        
        rooms[i].isDupeRoom = (!iSCE && i > 1 && (rand() % 100 < 15)); 
        
        if (rooms[i].isDupeRoom) {
            rooms[i].hasLeftRoom = false; rooms[i].hasFarRoom = false; rooms[i].hasRightRoom = false;
            ...
            rooms[i].dupeNumbers[0] = f1; rooms[i].dupeNumbers[1] = f2; rooms[i].dupeNumbers[2] = dN + 5; 
        ...
        if (prev.chosenExitSide == 0) rooms[i].orientation = (Direction)((prevDir + 3) % 4); 
        else if (prev.chosenExitSide == 1) rooms[i].orientation = prev.orientation;          
        else rooms[i].orientation = (Direction)((prevDir + 1) % 4);                          
        
        float stepX = 0, stepZ = 0;
        Direction exitGlobalDir = prev.orientation;
        if (prev.chosenExitSide == 0) exitGlobalDir = (Direction)((prevDir + 3) % 4);
        else if (prev.chosenExitSide == 2) exitGlobalDir = (Direction)((prevDir + 1) % 4);
        ...
        rooms[i].minX = rooms[i].centerX - 5.0f; rooms[i].maxX = rooms[i].centerX + 5.0f;
        rooms[i].minZ = rooms[i].centerZ - 5.0f; rooms[i].maxZ = rooms[i].centerZ + 5.0f;
        ...
        if (!rooms[i].isDupeRoom && !iSCE) {
            ...
            auto popSideRoom = [&](int* typeL, int* itemL, int* typeR, int* itemR) {
                ...
                for(int s=0; s<3; s++) {
                    if(s==1) { typeL[s]=99; itemL[s]=0; typeR[s]=99; itemR[s]=0; continue; } 
                    if(rand()%100 < 45) {
                        int r = rand()%100; typeL[s] = (r<25)?1:((r<50)?3:((r<75)?5:7));
                        if(typeL[s]==7) { if(cL) typeL[s]=5; else cL=true; }
                        itemL[s] = (typeL[s]>=3 && typeL[s]<=6 && rand()%100<30)?3:0;
                    } else { typeL[s]=0; itemL[s]=0; }
                    
                    if(rand()%100 < 45) {
                        int r = rand()%100; typeR[s] = (r<33)?2:((r<66)?6:8);
                        if(typeR[s]==8) { if(cR) typeR[s]=6; else cR=true; }
                        itemR[s] = (typeR[s]>=3 && typeR[s]<=6 && rand()%100<30)?3:0;
                    } else { typeR[s]=0; itemR[s]=0; }
                }
            };
