...
int main() {
    ...
    if (!vbo_main) {
        ...
        printf("Could not allocate graphics buffer.\n");
        printf("Press START to exit to Home Menu.\n");
        while (aptMainLoop()) {
            hidScanInput();
            if (hidKeysDown() & KEY_START) break;
            gfxFlushBuffers(); gfxSwapBuffers(); gspWaitForVBlank();
        }
        if(audio_ok) ndspExit(); C3D_TexDelete(&atlasTex); romfsExit(); C3D_Fini(); gfxExit(); return 0;
    }
